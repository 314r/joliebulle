...
var beerCalc = (function () {
    ...
    _gravityUnits = function (fermentables, batch_size, efficiency) {
        return (383.89 * _sugars(fermentables).mashedSugars / batch_size) * (efficiency / 100) + (383.89 * _sugars(fermentables).nonMashedSugars / batch_size);
    };

    _preBoilGravityUnits = function (fermentables, batch_size, efficiency) {
        return (383.89 * _sugars(fermentables).preBoilMashed / batch_size) * (efficiency / 100) + (383.89 * _sugars(fermentables).preBoilNonMashed / batch_size);
    };

    _preBoilGravity = function (fermentables, batch_size, efficiency) {
        return 1 + (_preBoilGravityUnits(fermentables, batch_size, efficiency) / 1000);
    };
...
/*      #Tinseth method
        ...
        */
        totalIbus = 0;
        bignessFactor = 1.65 * (Math.pow(0.000125, (_preBoilGravity(recipe.fermentables, recipe.batch_size, recipe.efficiency) - 1)));
        recipe.hops.forEach(function (hop) {
            ...
            mgAcid = (hop.alpha / 100) * (hop.amount * 1000) / recipe.batch_size;
    ...
    return {
        ...
        ebc : function (fermentables, batch_size) {
/*          calcul de la couleur
            ...
            mcuTot = 0;
            fermentables.forEach(function (fermentable) {
                mcu = 4.23 * fermentable.color * (fermentable.amount / 1000) / batch_size;
                mcuTot = mcuTot + mcu;
            });
        ...
        finalGravity : function (recipe) {
            ...
            hiAtten = hiAtten / 100;
            gu = _gravityUnits(recipe.fermentables, recipe.batch_size, recipe.efficiency) * (1 - hiAtten);
            return 1 + gu / 1000;
        },
        ...
        preBoilCalc : function (coolingLossRate, boilOffRate, boilTime, batch_size) {
            volPreCool = batch_size / (1 - coolingLossRate);
            volPreBoil = volPreCool / (1 - (boilOffRate * boilTime / 60));
            return volPreBoil;
        },

        preBoilSgCalc : function (gravityUnits, batch_size, volPreBoil) {
            ratio = batch_size / volPreBoil;
        ...
        grainfyieldCalc : function (grainWeight) {
            grainfyield = grainWeight * 1.5 / 1000;
            return grainVolume;
        },
